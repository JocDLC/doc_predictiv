...
- [~] B2. Implementar `readInputFile(file)`: **CSV listo** con detección de
      separador (`;` o `,`) y UTF-8 BOM. XLSX queda pendiente (B1: SheetJS).
- [x] B3. Implementar `parseInput(text)` → array de objetos fila.
- [x] B4. Implementar mapeo de columnas: autodetección por cabecera
...
- [x] B5. Input de texto "Identificador de campaña" (obligatorio) que se
...
- [x] C1. Implementar `normalizePhone(raw, country)` con la regla confirmada:
...
- [x] D1. Constante `WOLKVOX_HEADERS` con las 51 columnas exactas del template.
      Verificada automáticamente contra el template real.
- [x] D2. Implementar `mapToWolkvox(row)` con la convención confirmada:
...
- [x] D3. Implementar validación de lote:
      ...
      - `TEL1` duplicado entre leads → conservar el primero, excluir el resto
        e indicar el lead conservado.
- [x] D4. Implementar `downloadCSV()`: `;`, sin comillas automáticas, CRLF
      y UTF-8 BOM mediante `Blob`; réplica del formato de referencia aceptado
      por Wolkvox. Los valores que contengan `;`, comillas o saltos de línea
      se limpian para preservar las 51 columnas.
- [x] D5. Descarga con `Blob` + `<a download>`; nombre confirmado (P7):
      `predictivo_{PAIS}_{YYYYMMDD}.csv` (PAIS = ARG/COL/MEX).
- [x] D6. Reporte de excluidos: tabla en pantalla (fila original, lead ID,
...
- 2026-09-08: el archivo exacto rechazado por Wolkvox confirmó que el importador
  rechaza campos entre comillas. El export debe usar `;`, UTF-8 BOM, CRLF y
  sin comillas. Prueba Selenium con el CSV real: 472 leads, 51 columnas y
  TEL1 solo numérico.
